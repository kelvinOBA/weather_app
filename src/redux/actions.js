import { SAVE_WEATHER_DATA, 
    
    PREV_PAGE,
    SAVE_CARDS_TO_SHOW,
    NEXT_PAGE,
    SAVE_DEFAULT_ARRAY_LENGTH,
    START_LOADER, SAVE_DISTINCT_WEATHER,STOP_LOADER,CHANGE_UNIT } from "./types";
...
export const getWeatherData = () => (dispatch) => {
  ...
  let endpoint =
    `https://api.openweathermap.org/data/2.5/forecast?q=London,us&units=${store.getState().weatherDataReducer.unit}&appid=6166c722417ec3a451f59215e3e4599b`
  try {
    let formattedData;
    ...
    fetch(endpoint).then(function (response) {
      response.json().then(function (data) {
        ...
        let d2 = formattedData[0].dt_txt.split(' ');
